 ...
 * @author Radek
 */
public class HintonRBMGaussianBinary implements RBM {
    ...
    public HintonRBMGaussianBinary(RBMSettings rbmSettings, DataProvider dataProvider) {
    ...
    public void train() {
        ...
        for(int epoch = 0; epoch < maxepoch; epoch++) {
            ...
            for(int batch = 0; batch < numbatches; batch++) {
                ...
                // poshidstates = Gaussian Bernoulli
                FloatMatrix poshidstates = sigmoid((poshidprobs.add(FloatMatrix.randn(numcases, numhid).mul(0.1f))).neg()).gt(FloatMatrix.rand(numcases, numhid));
                
                if(epoch == maxepoch - 1) {
                    poshidstates = sigmoid(poshidprobs.neg());
                }
    ...
    public FloatMatrix getHidden(FloatMatrix visibleData) {
        return sigmoid(JCUDAMatrixUtils.multiply(visibleData.neg(), vishid, false, false).sub(hidbiases.repmat(visibleData.getRows(), 1))).gt(FloatMatrix.rand(numcases, numhid));
    }
